class RayysMouse {
    ...
    updateMouseCoords(event) {
      ...
      this.rawCoords.x =  (event.clientX - this.renderer.domElement.offsetLeft)      - this.renderer.domElement.offsetWidth/2;///2;
      this.rawCoords.y = -(event.clientY - this.renderer.domElement.offsetTop + 0.5) + this.renderer.domElement.offsetHeight/2;///2;
      this.mouse.x =  ((event.clientX - this.renderer.domElement.offsetLeft + 0.5) / this.renderer.domElement.offsetWidth)  *2 - 1; //2
...
//function selectObjects(ojects, frustum) {
  function selectObjects(d_Objects, frustum) {
    ...
    if(parameters3.h==true){
      //console.log("gebouwfunctie aanwijzen");
              ...
              for (let key of Object.keys(d_Objects)) {
               // d_Objects[key].geometry.computeBoundingBox();
               // if (frustum.intersectsBox(d_Objects[key].bbox)) {
                if (frustum.intersectsBox(d_Objects[key].geometry.boundingBox)) {
                 // console.log("a hit");
                  
                  if (!d_Objects[key].selected) {
                    //objects[key].obj.material = selectedMaterial;
                    d_Objects[key].material = materialFunction;
                    d_Objects[key].colorFunction = materialFunction;  //storing for longer use
                    //console.log(objects[key].colorFunction);
                   // console.log(d_Objects[key].id);  //   extract correct id
                    spacesID.push(d_Objects[key].id);  //   puts the  id in array to select
                    
  
  
  
                  }
                  d_Objects[key].selected = true;
                } else {
                  if (d_Objects[key].selected) {
                    //objects[key].obj.material = defaultMaterial;
                    spacesID.length=0; // make array with temp objects id blank
                  }
                  d_Objects[key].selected = false;
                }
              }
    }
    else {
     ...
     // console.log("cluster aanwijzen");
      for (let key of Object.keys(d_Objects)) {
        ...
        if (frustum.intersectsBox(d_Objects[key].geometry.boundingBox)) {
          if (!d_Objects[key].selected) {
            //objects[key].obj.material = selectedMaterial;
            d_Objects[key].material = materialCluster;
            d_Objects[key].colorCluster = materialCluster;  //storing for longer use
            d_Objects[key].cluster= functionCluster;
            //console.log(objects[key].colorFunction);
           // console.log(d_Objects[key].id);  //   extract correct id
            spacesID.push(d_Objects[key].id);  //   puts the  id in array to select
